...
export default function LinksPage(){
 ...
 <section className="card tablewrap"><table className="table"><thead><tr><th>Short link</th><th>Destination</th><th>Clicks</th><th>Status</th><th>Actions</th></tr></thead><tbody>{links.map(link=><tr key={link.id}><td><a className="linkurl" href={'/r/'+link.slug} target="_blank">/r/{link.slug}</a>{link.expiresAt&&<small className="muted">Expires {new Date(link.expiresAt).toLocaleDateString()}</small>}</td><td><div className="truncate">{link.targetUrl}</div></td><td>{link.clicks}</td><td><span className="pill">{link.status||(link.active?'ACTIVE':'PAUSED')}</span></td><td><div className="rowactions"><Link className="rowactions button" href={'/dashboard/links/'+encodeURIComponent(link.slug)}>Edit</Link><button disabled={busy} onClick={()=>toggle(link)}>{link.active?'Pause':'Activate'}</button><button disabled={busy} onClick={()=>remove(link)}>Delete</button></div></td></tr>)}{!links.length&&<tr><td colSpan={5} className="empty muted">No links yet.</td></tr>}</tbody></table></section></main>
}
